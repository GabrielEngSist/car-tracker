...
{
    public async Task<HandlerResult<bool>> Handle(DeleteMaintenancePlanItemCommand request, CancellationToken cancellationToken)
    {
        ...
        if (item is null) return RequestOutcome.Ok(false);
